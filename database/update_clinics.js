...
async function updateClinics() {
  ...
  try {
    ...
    console.log('✅ Successfully updated clinics with Mati City facilities');
    
    // Verify the updates
    const result = await client.query('SELECT name, address FROM clinics ORDER BY id');
    console.log('\n📋 Updated Clinics:');
    result.rows.forEach((clinic, index) => {
